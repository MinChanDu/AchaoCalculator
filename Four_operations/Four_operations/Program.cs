...
namespace Four_arimetic
{
    class Operation
    {
        ...
        {
            int p, t;
            int[] arr = getRandomNum(4, 1, 100); //从1至100中取出4个互不相同的随机数
            int[] arr1 = getRandomNum(3, 1, 50);//取出随机数决定操作符
            int[] b = new int[1];//存储每次操作符后的结果
            char[] c = new char[3];//存储确定的操作符
            string d;//存储算式及结果，后续输入到文件中
            p = arr1[0] % 3;
            switch (p)
            {
                case 0:
            ...
            Console.WriteLine("{0}{1}{2}{3}{4}{5}{6}=", arr[0], c[0], arr[1], c[1], arr[2], c[2], arr[3]);
            d = Convert.ToString(arr[0]) + Convert.ToString(c[0]) + Convert.ToString(arr[1]) + Convert.ToString(c[1]) + Convert.ToString(arr[2]) + Convert.ToString(c[2] + Convert.ToString(arr[3])) + "=" + Convert.ToString(b[0]);
            PutFile(d);
        }
    }
    class Program
